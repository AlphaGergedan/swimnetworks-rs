...
use swimnetworks_rs::{random_feature, Activation, Layer, ModelConfig, RandomFeatureSamplerConfig, Sample, SamplerConfig};
...
fn fit(rng_lower: Input, rng_upper: Input, rate: Input, target_fn: TargetFunction) {
    ...
    println!("-> L2 error after zero-initializing the parameters: {}", l2_error(truths.clone(), outputs.clone()));
    ...
    let outputs = model.forward(inputs.clone());
    println!("-> L2 error after random-feature sampling: {}", l2_error(truths.clone(), outputs.clone()));
    ...
    let outputs = model.forward(inputs.clone());
    println!("-> L2 error after the last linear layer fitting lstsq: {}", l2_error(truths, outputs));
}
...
fn main() {
    ...
    const RNG_LOWER: Input = -2.;
    const RNG_UPPER: Input = 2.;
...
pub fn l2_error(input: Array2<Input>, output: Array2<Output>) -> f64 {
    (output - input).norm()
}
